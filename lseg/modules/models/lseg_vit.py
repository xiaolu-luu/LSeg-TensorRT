...
def forward_vit(pretrained, x):
    ...
    layer_1 = pretrained.activations["1"] #torch.Size([1, 901, 1024])
    layer_2 = pretrained.activations["2"]
    layer_3 = pretrained.activations["3"]
    layer_4 = pretrained.activations["4"]

    layer_1 = pretrained.act_postprocess1[0:2](layer_1) # torch.Size([1, 1024, 900])
    layer_2 = pretrained.act_postprocess2[0:2](layer_2)
    layer_3 = pretrained.act_postprocess3[0:2](layer_3)
    layer_4 = pretrained.act_postprocess4[0:2](layer_4)

    # 确保patch_size是静态的和确定的
    patch_size_h = pretrained.model.patch_size[1]
    patch_size_w = pretrained.model.patch_size[0] # 16
    unflattened_size_h = h // patch_size_h # 30
    unflattened_size_w = w // patch_size_w

    # 创建一个静态的整数元组
    int_value1 = int(unflattened_size_h)#.item()
    int_value2 = int(unflattened_size_w)#.item()

    unflattened_size = (int_value1, int_value2)
    ...
    if layer_1.ndim == 3:
        layer_1 = unflatten(layer_1) # torch.Size([1, 1024, 30, 30])
    if layer_2.ndim == 3:
    ...
    layer_1 = pretrained.act_postprocess1[3 : len(pretrained.act_postprocess1)](layer_1) # torch.Size([1, 256, 120, 120])
